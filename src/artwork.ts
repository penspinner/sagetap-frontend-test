export const artwork = {
  data: {
    id: 27993,
    api_model: 'artworks',
    api_link: 'https://api.artic.edu/api/v1/artworks/27993',
    is_boosted: false,
    title: 'The Market-Woman',
    alt_titles: null,
    thumbnail: {
      lqip: 'data:image/gif;base64,R0lGODlhAwAFAPMAAH59boB+b4OAcIKBcYWDc4aEdIiGdoqIeY2Me46Me5qXgZyZiJ+ci6CdiqGfjgAAACH5BAAAAAAALAAAAAADAAUAAAQLUDXHElrhiGLAIBEAOw==',
      width: 1582,
      height: 2400,
      alt_text: 'A work made of engraving in black on ivory laid paper.',
    },
    main_reference_number: '1938.1356.1',
    has_not_been_viewed_much: true,
    boost_rank: null,
    date_start: 1537,
    date_end: 1547,
    date_display: 'c. 1542',
    date_qualifier_title: "Artist's working dates",
    date_qualifier_id: 15,
    artist_display: 'Sebald Beham\nGerman, 1500-1550',
    place_of_origin: 'Germany',
    dimensions: 'Image/sheet, trimmed to platemark: 3.9 × 2.6 cm (1 9/16 × 1 1/16 in.)',
    medium_display: 'Engraving in black on ivory laid paper',
    inscriptions: null,
    credit_line: 'Clarence Buckingham Collection',
    catalogue_display: '<p>Hollstein 188 II/II</p><p>Bartsch 187</p><p>Pauli 188</p>',
    publication_history: null,
    exhibition_history: null,
    provenance_text: null,
    publishing_verification_level: 'Web Basic',
    internal_department_id: 3,
    fiscal_year: null,
    fiscal_year_deaccession: null,
    is_public_domain: true,
    is_zoomable: true,
    max_zoom_window_size: -1,
    copyright_notice: null,
    has_multimedia_resources: false,
    has_educational_resources: false,
    has_advanced_imaging: false,
    colorfulness: 11.2002,
    color: {
      h: 52,
      l: 53,
      s: 8,
      percentage: 0.08310169662872369,
      population: 896,
    },
    latitude: null,
    longitude: null,
    latlon: null,
    is_on_view: false,
    on_loan_display: null,
    gallery_title: null,
    gallery_id: null,
    artwork_type_title: 'Print',
    artwork_type_id: 18,
    department_title: 'Prints and Drawings',
    department_id: 'PC-13',
    artist_id: 33577,
    artist_title: 'Hans Sebald Beham',
    alt_artist_ids: [],
    artist_ids: [33577],
    artist_titles: ['Hans Sebald Beham'],
    category_ids: ['PC-13'],
    category_titles: ['Prints and Drawings'],
    term_titles: ['engraving', 'paper (fiber product)', 'print', 'prints and drawing'],
    style_id: null,
    style_title: null,
    alt_style_ids: [],
    style_ids: [],
    style_titles: [],
    classification_id: 'TM-12',
    classification_title: 'engraving',
    alt_classification_ids: ['TM-17', 'TM-4'],
    classification_ids: ['TM-12', 'TM-17', 'TM-4'],
    classification_titles: ['engraving', 'print', 'prints and drawing'],
    subject_id: null,
    alt_subject_ids: [],
    subject_ids: [],
    subject_titles: [],
    material_id: 'TM-2982',
    alt_material_ids: [],
    material_ids: ['TM-2982'],
    material_titles: ['paper (fiber product)'],
    technique_id: null,
    alt_technique_ids: [],
    technique_ids: [],
    technique_titles: [],
    theme_titles: [],
    image_id: '592cd558-bc18-aaac-6256-b9ea60d64a51',
    alt_image_ids: [],
    document_ids: [],
    sound_ids: [],
    video_ids: [],
    text_ids: [],
    section_ids: [],
    section_titles: [],
    site_ids: [],
    suggest_autocomplete_all: [
      {
        input: ['1938.1356.1'],
        contexts: {
          groupings: ['accession'],
        },
      },
      {
        input: ['The Market-Woman'],
        weight: 124,
        contexts: {
          groupings: ['title'],
        },
      },
    ],
    source_updated_at: '2022-09-21T19:18:50-05:00',
    updated_at: '2022-10-02T16:19:52-05:00',
    timestamp: '2022-11-08T09:21:42-06:00',
  },
  info: {
    license_text:
      'The data in this response is licensed under a Creative Commons Zero (CC0) 1.0 designation and the Terms and Conditions of artic.edu.',
    license_links: [
      'https://creativecommons.org/publicdomain/zero/1.0/',
      'https://www.artic.edu/terms',
    ],
    version: '1.5',
  },
  config: {
    iiif_url: 'https://www.artic.edu/iiif/2',
    website_url: 'http://www.artic.edu',
  },
}

export type Artwork = typeof artwork
